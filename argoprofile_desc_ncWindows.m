...
%   is missing)
if(isfield(fp,'p_oxygen') && isempty(fp.p_desc_raw) && isempty(fp.p_oxygen))
    return
elseif (~isfield(fp,'p_oxygen') && isempty(fp.p_desc_raw))
    return
end

% DATA_MODE could be set by testing for non-empty s_calibrate, but for now
% fix to not 'adjusted' for descending profiles (?).  Note - if the primary 
% profile is empty, then this gets
% messy.
...
for ii = 1:nin
    ...
    if ii==1  % handle multiprofile files here with if/else
        ...
        if nlv>0
         %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%ADD20160719
%         fp=calibrate_p_desc(fp);
        fp = qc_desc_tests(dbdat,fp);
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%           
            
            netcdf.putVar(ncid,NPRESID,[0,ii-1],[length(nan2fv(fp.p_desc_raw(irev),fval)),1],nan2fv(fp.p_desc_raw(irev),fval)); 
            netcdf.putVar(ncid,NPRESQCID,[0,ii-1],[length(num2str(fp.p_desc_qc(irev),'%1d')),1],nan2fv(num2str(fp.p_desc_qc(irev),'%1d'))); 
            
            if isempty(fp.p_desc_calibrate)
                if ispc
...
%                 [float,cal_rep] = calsal(float,np);
            ...
            if adjusted
            pc=qc_apply4(fp.p_desc_calibrate(irev),fp.p_desc_qc(irev));

            netcdf.putVar(ncid,NPRESADID,[0,ii-1],[length(nan2fv(pc,fval)),1],nan2fv(pc,fval)); 
            netcdf.putVar(ncid,NPRESADQCID,[0,ii-1],[length(num2str(fp.p_desc_qc(irev),'%1d')),1],num2str(fp.p_desc_qc(irev),'%1d')); 

            qcflag = overall_qcflag(fp.p_desc_qc);
            netcdf.putVar(ncid,NPROPRESQCID,ii-1,length(qcflag),qcflag); 

        end
        
        if ~isempty(fp.t_desc_raw)
            netcdf.putVar(ncid,NTEMPID,[0,ii-1],[length(nan2fv(fp.t_desc_raw(irev),fval)),1],nan2fv(fp.t_desc_raw(irev),fval)); 
            netcdf.putVar(ncid,NTEMPQCID,[0,ii-1],[length(num2str(fp.t_desc_qc(irev),'%1d')),1],num2str(fp.t_desc_qc(irev),'%1d')); 

           if adjusted
                if ~isempty(fp.t_desc_calibrate)
                    tc=qc_apply4(fp.t_desc_calibrate(irev),fp.t_desc_qc(irev));
                    netcdf.putVar(ncid,NTEMPADID,[0,ii-1],[length(nan2fv(tc,fval)),1],nan2fv(tc,fval));
                    
                else
                    tc=qc_apply4(fp.t_desc_raw(irev),fp.t_desc_qc(irev));
                    netcdf.putVar(ncid,NTEMPADID,[0,ii-1],[length(nan2fv(tc,fval)),1],nan2fv(tc,fval));
                    
                end
                netcdf.putVar(ncid,NTEMPADQCID,[0,ii-1],[length(num2str(fp.t_desc_qc(irev),'%1d')),1],num2str(fp.t_desc_qc(irev),'%1d'));
            end
            qcflag = overall_qcflag(fp.t_desc_qc);
            netcdf.putVar(ncid,NPROTEMPQCID,ii-1,length(qcflag),qcflag);
            
        end
        
        if ~isempty(fp.s_desc_raw)
            netcdf.putVar(ncid,NPSALID,[0,ii-1],[length(nan2fv(fp.s_desc_raw(irev),fval)),1],nan2fv(fp.s_desc_raw(irev),fval)); 
            netcdf.putVar(ncid,NPSALQCID,[0,ii-1],[length(num2str(fp.s_desc_qc(irev),'%1d')),1],num2str(fp.s_desc_qc(irev),'%1d')); 

            if adjusted
                sc=qc_apply4(fp.s_desc_calibrate(irev),fp.s_desc_qc(irev));
                netcdf.putVar(ncid,NPSALADID,[0,ii-1],[length(nan2fv(sc,fval)),1], nan2fv(sc,fval));
                netcdf.putVar(ncid,NPSALADQCID,[0,ii-1],[length(num2str(fp.s_desc_qc(irev),'%1d')),1],num2str(fp.s_desc_qc(irev),'%1d'));
            end
            qcflag = overall_qcflag(fp.s_desc_qc);
    ...
    elseif ii==2
        ...
        priqcP=find(fp.p_desc_qc>=3 & fp.p_desc_qc<=4);
        priqcT=find(fp.t_desc_qc>=3 & fp.t_desc_qc<=4);
        priqcS=find(fp.s_desc_qc>=3 & fp.s_desc_qc<=4);
        
        if ~isempty(priqcP)
            ppbad=range(fp.p_desc_calibrate(priqcP));
        ...
        if ~isempty(priqcT)
            if max(diff(priqcT)<=1)
                ppbad=range(fp.p_desc_calibrate(priqcT));
                ppreject=find(p2 > ppbad(1) & p2 < ppbad(2));
                QC.t(ppreject)=3;
            end
        end
        ...
        if ~isempty(priqcS)
            if max(diff(priqcS)<=1)
                ppbad=range(fp.p_desc_calibrate(priqcS));
                ppreject=find(p2 > ppbad(1) & p2 < ppbad(2));
                QC.s(ppreject)=3;
            end
        end
    ...
    if ii==1 | (ii==2 & dbdat.subtype~=1005)
        ...
        for par = pars
            ...
            for kk = 1:length(kst)
                ...
                if ii==1
                    if ~isempty(fp.p_desc_calibrate) & ~isnan(fp.p_desc_calibrate(kend(kk)))

                        netcdf.putVar(ncid,NHISSTAPRESID,[ii-1,jj-1],[length(fp.p_desc_calibrate(kend(kk))),1],fp.p_desc_calibrate(kend(kk)));                           
                    end
                else
                ...
                if ii==1
                    if  ~isempty(fp.p_desc_calibrate) & ~isnan(fp.p_desc_calibrate(kst(kk)))
                        netcdf.putVar(ncid,NHISSTOPPRESID,[ii-1,jj-1],[length(fp.p_desc_calibrate(kst(kk))),1],fp.p_desc_calibrate(kst(kk)));                           
                    end
                else
                    if ~isnan(p_c(kst(kk)))
        ...
        if dbdat.oxy 
            argoprofile_Bfile_ncWindows(dbdat,fp);
        end
