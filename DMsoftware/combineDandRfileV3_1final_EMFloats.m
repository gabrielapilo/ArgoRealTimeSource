    ...
    if ~isempty(fpp)
        ...
        for iFile = 1:length(fpp)
            ...
            if exist(fD,'file')
                ...
                fR = sprintf('%s%i/R%i_%03iD.nc', Rpath, dbdat.wmo_id,...
                    dbdat.wmo_id, fpp(iFile).profile_number);
                if exist(fR,'file')
                    fDO = sprintf('%s%i/D%i_%03iD.nc', DpathOutput, dbdat.wmo_id,...
                        dbdat.wmo_id, fpp(iFile).profile_number);
                else
...
%                             else   % go and fill the old R with the new D fields:
                                getAndSetNetCdfField(ncDfile, ncRfile, 'DATA_TYPE', 0);
