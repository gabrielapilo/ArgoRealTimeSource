...
for ii = ipf(:)'
   ...
   if isempty(fp.p_desc_raw) & isempty(fp.s_desc_raw) & isempty(fp.t_desc_raw)
...
if isfield(fp,'p_oxygen') & ~isempty(fp.p_oxygen)
   ...
   if  ~isempty(fp.p_desc_raw)
      fp.p_desc_qc = ones(size(fp.p_desc_raw),'uint16');
      jj = find(isnan(fp.p_desc_raw));
      fp.p_desc_qc(jj) = 9;
   end
   if  ~isempty(fp.t_desc_raw)
      fp.t_desc_qc = ones(size(fp.t_desc_raw),'uint16');
      jj = find(isnan(fp.t_desc_raw));
      fp.t_desc_qc(jj) = 9;
   end
   if  ~isempty(fp.s_desc_raw)
      fp.s_desc_qc = ones(size(fp.s_desc_raw),'uint16');
      jj = find(isnan(fp.s_desc_raw));
      fp.s_desc_qc(jj) = 9;
   end
   ...
   jj=find(fp.p_desc_raw>dbdat.profpres+(dbdat.profpres*.1));

   if ~isempty(jj)
      newv = repmat(4,1,length(jj));
      fp.p_desc_qc(jj) = max([fp.p_desc_qc(jj); newv]);
      fp.t_desc_qc(jj) = max([fp.t_desc_qc(jj); newv]);
      fp.s_desc_qc(jj) = max([fp.s_desc_qc(jj); newv]);
      fp.p_desc_calibrate(jj)=NaN;
      fp.testsfailed(19) = 1;      
...
if (~isnan(fp.lat(1)))
   ...
   jj = find(fp.t_desc_raw<=-3.5 | fp.t_desc_raw>40.);
   kk = find(fp.s_desc_raw<2.0 | fp.s_desc_raw>41.);
   if ~isempty(jj)
      newv = repmat(4,1,length(jj));
      fp.t_desc_qc(jj) = max([fp.t_desc_qc(jj); newv]);
      fp.testsfailed(6) = 1;
   ...
   if ~isempty(kk)
      ...
      fp.s_desc_qc(kk) = max([fp.s_desc_qc(kk); newv]);
      fp.testsfailed(6) = 1;
   end
   ...
   gg = find(~isnan(fp.p_desc_calibrate));
   if any(diff(fp.p_desc_calibrate(gg))==0)
       fp.testsfailed(8) = 1;
       jj=(diff(fp.p_desc_calibrate(gg))==0);
...
%      
%       bb = [];
%       lp = fp.p_desc_calibrate(gg(1));
%       for jj = 2:length(gg)
%          if fp.p_desc_calibrate(gg(jj)) < lp
%             lp = fp.p_desc_calibrate(gg(jj));
%          else
%             bb = [bb gg(jj)];
%          end
%       end
%       newv = repmat(4,1,length(bb));
%       fp.s_desc_qc(bb) = max([fp.s_desc_qc(bb); newv]);
%       fp.t_desc_qc(bb) = max([fp.t_desc_qc(bb); newv]);
%       fp.p_desc_qc(bb) = max([fp.p_desc_qc(bb); newv]);
%    end
% 
...
bb=[];
kk=find(diff(fp.p_desc_calibrate)>0);

if length(kk)>0 
    for jj=1:length(kk)
       for l=kk(jj):kk(jj)+1    %max(2,kk(jj)):min(length(fp.p_desc_calibrate)-2,kk(jj)+1)
           if l>=length(fp.p_desc_calibrate)-1
               bb=[bb min(length(fp.p_desc_calibrate),l+1)];
           elseif l==1 
               if fp.p_desc_calibrate(l)< fp.p_desc_calibrate(l+2)
                bb=[bb l];               
               else
                   bb=[bb l+1];
               end      
           elseif(fp.p_desc_calibrate(l)>=fp.p_desc_calibrate(l-1) | fp.p_desc_calibrate(l)<= fp.p_desc_calibrate(l+2))
               bb=[bb l];
           end
       end
   end
   ...
   bdt = findspike(fp.t_desc_raw,fp.p_desc_raw,'t');
   if ~isempty(bdt)
      newv = repmat(3,1,length(bdt));
      fp.t_desc_qc(bdt) = max([fp.t_desc_qc(bdt); newv]);
      fp.testsfailed(9) = 1;
   end

   bds = findspike(fp.s_desc_raw,fp.p_desc_raw,'s');
   if ~isempty(bds)
      newv = repmat(3,1,length(bds));
      fp.s_desc_qc(bds) = max([fp.s_desc_qc(bds); newv]);
      fp.testsfailed(9) = 1;
   end
   if dbdat.oxy
       if length(fp.oxy_raw)~=length(fp.p_desc_raw)
           po=fp.p_oxygen;
       else
           po=fp.p_desc_raw;
       end
   ...
   %  generally
    if(fp.wmo_id==1901121)
       ...
       [bdt,bbt] = findspike(fp.t_desc_raw,fp.p_desc_raw,'t');
       if ~isempty(bbt)
          newv = repmat(4,1,length(bbt));
          fp.t_desc_qc(bbt) = max([fp.t_desc_qc(bbt); newv]);
          fp.testsfailed(9) = 1;
       end

       [bds,bbs] = findspike(fp.s_desc_raw,fp.p_desc_raw,'s');
       if ~isempty(bbs)
          newv = repmat(4,1,length(bbs));
          fp.s_desc_qc(bbs) = max([fp.s_desc_qc(bbs); newv]);
          fp.testsfailed(9) = 1;
       end
   ...
   if nlev>=3
       ...
       testv = abs(fp.t_desc_raw(jj) - (fp.t_desc_raw(jj+1)+fp.t_desc_raw(jj-1))/2);
       kk = find(testv>9 | (fp.p_desc_raw(jj)>500 & testv>3));
       if ~isempty(kk)
           newv = repmat(3,1,length(kk));
           fp.t_desc_qc(kk+1) = max([fp.t_desc_qc(kk+1); newv]);
           fp.testsfailed(11) = 1;
       end
       
       testv = abs(fp.s_desc_raw(jj) - (fp.s_desc_raw(jj+1)+fp.s_desc_raw(jj-1))/2);
       kk = find(testv>1.5 | (fp.p_desc_raw(jj)>500 & testv>0.5));
       if ~isempty(kk)
           newv = repmat(3,1,length(kk));
           fp.s_desc_qc(kk+1) = max([fp.s_desc_qc(kk+1); newv]);
           fp.testsfailed(11) = 1;
       end
   ...
   jj = find(diff(fp.t_desc_raw)>10.);
   kk = find(diff(fp.s_desc_raw)>5.);
   
   if ~isempty(jj)
       newv = repmat(4,1,length(jj));
       fp.t_desc_qc(jj+1) = max([fp.t_desc_qc(jj+1); newv]);
       fp.testsfailed(12) = 1;
   end
   if ~isempty(kk)
       newv = repmat(4,1,length(kk));
   ...
   if ~isempty(fp.s_desc_raw) && all(fp.s_desc_raw==fp.s_desc_raw(1))
       newv = repmat(4,1,nlev);
       fp.s_desc_qc(1:nlev) = max([fp.s_desc_qc(1:nlev); newv]);
       fp.testsfailed(13) = 1;
   end
   if ~isempty(fp.t_desc_raw) && all(fp.t_desc_raw==fp.t_desc_raw(1))
       newv = repmat(4,1,nlev);
       fp.t_desc_qc(1:nlev) = max([fp.t_desc_qc(1:nlev); newv]);
      fp.testsfailed(13) = 1;
   end
   if dbdat.oxy
...
%    density = sw_pden(fp.s_desc_raw,fp.t_desc_raw,fp.p_desc_calibrate,0);
%    dd = diff(density);
% 
%    jj = find(dd>0.04);
%    if (~isempty(jj) & fp.s_desc_qc(jj)<=2 & fp.t_desc_qc(jj)<=2)
%       % Have to reject value at both levels involved
%       kk = unique([jj; jj+1]);
%       newv = repmat(4,1,length(kk));
%       fp.t_desc_qc(kk) = max([fp.t_desc_qc(kk); newv]);
%       fp.s_desc_qc(kk) = max([fp.s_desc_qc(kk); newv]);
%       fp.testsfailed(14) = 1;
%    end
...
% not surface reference level...:
       ...
       difdd=0;
       for iij=1:length(fp.p_desc_calibrate)-1
          difdd(iij)=0;
           
           density = sw_pden(fp.s_desc_raw(iij:iij+1),fp.t_desc_raw(iij:iij+1),fp.p_desc_calibrate(iij:iij+1), ...
               (fp.p_desc_calibrate(iij)+fp.p_desc_calibrate(iij+1))/2);
      ...
      for iij=length(fp.p_desc_calibrate):-1:2
          difdd(iij)=0;
           
           density = sw_pden(fp.s_desc_raw(iij:-1:iij-1),fp.t_desc_raw(iij:-1:iij-1),fp.p_desc_calibrate(iij:-1:iij-1), ...
               (fp.p_desc_calibrate(iij)+fp.p_desc_calibrate(iij-1))/2);
           difdd(iij)=diff(density);
       ...
       if (~isempty(jf))
           ...
           fp.t_desc_qc(jf) = max([fp.t_desc_qc(jf); newv]);
           fp.s_desc_qc(jf) = max([fp.s_desc_qc(jf); newv]);
           fp.testsfailed(14) = 1;
       end
   ...
   if(strcmp(dbdat.status,'suspect') | strcmp(dbdat.status,'evil'))
      ...
      fp.s_desc_qc(vv) = max([fp.s_desc_qc(vv); newv]);
%            fp.s_desc_qc(vv) = 3;
           vvs = qc_apply(fp.s_desc_raw,fp.s_desc_qc);
       else
      ...
      fp.s_desc_qc(vv) = max([fp.s_desc_qc(vv); newv]);
      fp.p_desc_qc(vv) = max([fp.p_desc_qc(vv); newv]);
      fp.t_desc_qc(vv) = max([fp.t_desc_qc(vv); newv]);
%                fp.p_desc_qc(vv) = 3;
%                fp.s_desc_qc(vv) = 3;
%                fp.t_desc_qc(vv) = 3;
           end
           pii = qc_apply(fp.p_desc_calibrate,fp.p_desc_qc);
           vvt = qc_apply(fp.t_desc_raw,fp.t_desc_qc);
           vvs = qc_apply(fp.s_desc_raw,fp.s_desc_qc);
       end       
   end
   ...
   if ii>1
      ...
      pll = qc_apply(fpp(ll).p_desc_calibrate,fpp(ll).p_qc);
...
%          while (maxp<500 && ll>2) | isnan(ddsp)
        ...
        if maxp<500 | isnan(ddsp)
          ...
          if ~isempty(maxp) && maxp>500 && ~isnan(ddsp)
             ...
             if ~isempty(jj) && ~isempty(kk)
                vv = qc_apply(fp.t_desc_raw,fp.t_desc_qc);
                ddtm = nanmean(vv(jj));
                vv = qc_apply(fpp(ll).t_raw,fpp(ll).t_qc);
                ddtp = nanmean(vv(kk));
                vv = qc_apply(fp.s_desc_raw,fp.s_desc_qc);
                ddsm = nanmean(vv(jj));
                vv = qc_apply(fpp(ll).s_raw,fpp(ll).s_qc);
                ddsp = nanmean(vv(kk));
       ...
       if ~isempty(latc)
           if lstp<1 || isempty(fpp(lstp).lat)  || latc<-65
               ...
               if(isempty(fp.p_desc_raw))
                   ...
                   gg=1:50:fp.p_desc_raw(1);
                   avgt=[];
                   avglt=[];
                   avgs=[];
                   avgls=[];
                   for i=1:length(gg)-1
                       kk=find(fp.p_desc_raw>=gg(i) & fp.p_desc_raw<=gg(i+1) & fp.t_desc_raw<9999);
                       kklp=find(fpp(lstp).p_raw>=gg(i) & fpp(lstp).p_raw<=gg(i+1) & fpp(lstp).t_raw<9999);
                       avgt(i)=nanmean(fp.t_desc_raw(kk));
                       avgs(i)=nanmean(fp.s_desc_raw(kk));
               ...
               if(~isempty(avgt))
                   ...
                   if(~isempty(mmT) & mmT(1)<0.0001 & mmT(2)<0.3 & mdT<0.01 )
                       fp.testsfailed(18)=1;
                       newv = repmat(3,1,length(fp.t_desc_qc));
                       fp.t_desc_qc=newv;
                   end
   ...
   if max(fp.p_desc_raw) < dbdat.profpres*.95
      fp.grounded = 'Y';
   else
      fp.grounded = 'N';
   end
   ...
   if isfield(fp,'oxyT_raw')  %
       ...
       elseif length(fp.p_desc_raw)==length(fp.oxyT_raw)
       QC = qc_tests_Profile2(dbdat,fp.p_desc_raw,fp.s_desc_raw,fp.oxyT_raw, ...
           fp.p_desc_qc,fp.s_desc_qc,fp.oxyT_qc);
       end
%        fp.p_oxygen_qc=QC.p;
       fp.oxyT_qc=QC.t;
%        fp.s_oxygen_qc=QC.s;
