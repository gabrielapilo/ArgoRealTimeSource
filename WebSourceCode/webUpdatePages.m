%=========================================================================================
% MAIN PROGRAM TO GENERATE AND UPDATE HTML WEB PAGES.
% CONSTRUCTS ALL WEB PAGES FOR ALL FLOATS SPECIFIED IN ArgoRT/matfiles/floatWMO.mat
%
% Usage:
%       wmoID:  If specified, then a single float web page is re-generated and updated.
%               Index pages are also updated.
%
%               If empty, then all web pages for all floats are re-generated and updated.
%               Index pages are also updated.
%
% usage: function webUpdatePages(wmoID,float)
%==========================================================================================
function webUpdatePages(wmoID,rebuild)
%begin
    %DEFINE GLOBAL DIRECTORIES:
    global FOLDER_HTML  FOLDER_DBASE % FOLDER_CONFIG
    global ARGO_SYS_PARAM
    
    %load all filenames of deployed floats
    warning off;
    if nargin<2 || isempty(rebuild)
        rebuild = 0;
    end
    
       %location of destination and two source folders:
    FOLDER_HTML   = ARGO_SYS_PARAM.Folder_html;              %destination web files
    FOLDER_DBASE  = ARGO_SYS_PARAM.Folder_Dbase;            %dbase source files  ex: float1901119.mat
%     FOLDER_CONFIG = ARGO_SYS_PARAM.Folder_Config;            %config source files ex: 3551_Config.mat
    
    %update all floats:
%     if (isempty(wmoID))   AllWebPages();     return; end;
    
%     if (~exist('float','var'))
%         float=getargo(wmoID);
%     end
    try
        A = webMakeSingleWebPage(wmoID,rebuild);
        
        if (isempty(A))
            fprintf('WMO ID not found: %d \n', wmoID);
            return;
        end
        %update only specified float with wmo number:
        %     if (isnumeric(wmoID)) OneWebPage(wmoID,float); return; end;
...
%####################################### INDEX TABLE HTML GENERATION HELPER FUNCTIONS #########################################


%====================================================================
% RE-GENERATES AND UPDATES ALL WEB PAGES FOR ONE FLOAT WITH wmoID
% INDEX PAGES ARE ALSO UPDATED:
%====================================================================
% function OneWebPage(wmoID,float)
% %begin
%     %DEFINE GLOBAL DIRECTORIES:
%     global FOLDER_HTML  FOLDER_DBASE  %FOLDER_CONFIG
%     global ARGO_SYS_PARAM
...
% 
%     %update one web page
%     A = webMakeSingleWebPage(wmoID,float);
%     
%     if (isempty(A)) 
%         fprintf('WMO ID not found: %d \n', wmoID);
%         return;
%     end
%     
%     %this .mat data file has index page table entry information for all floats:
% %     load IndexPageInfo;  useless! not updated.
% 
% %     %look for the specific hull ID for this updated float:
% %     try n=length(IndexPageInfo); catch; return; end;
% %     found = false;
% %     
% %     for j=1:n
% %         if (A.HullID==IndexPageInfo(j).HullID)
% %             IndexPageInfo(j)= A;
% %             found  = true;
% %             break;
% %         end
% %     end
%     
% %     %not found, new float, append to structure array:, save to .mat file:
% %     if (~found) IndexPageInfo(n+1)=A; end;
% %     save('IndexPageInfo.mat', 'IndexPageInfo');
%     
%     %generate the index pages sorted in different columns:
% %     folder_dest = FOLDER_HTML;
% %     fprintf(' \n');
% %     fprintf('CONSTRUCTING INDEX PAGES: \n');
% %     fprintf('------------------------- \n');  
%     HTMLmakeIndexPage([ARGO_SYS_PARAM.folder_dest, 'index.html'],                   IndexPageInfo, 'HULLID'         );
%     HTMLmakeIndexPage([ARGO_SYS_PARAM.folder_dest, 'IndexAU_HullID.html'],          IndexPageInfo, 'HULLID'         );
%     HTMLmakeIndexPage([ARGO_SYS_PARAM.folder_dest, 'IndexAU_ArgosID.html'],         IndexPageInfo, 'ARGOSID'        );
%     HTMLmakeIndexPage([ARGO_SYS_PARAM.folder_dest, 'IndexAU_WmoID.html'],           IndexPageInfo, 'WMOID'          );
%     HTMLmakeIndexPage([ARGO_SYS_PARAM.folder_dest, 'IndexAU_LaunchDate.html'],      IndexPageInfo, 'LaunchDate'     );
%     HTMLmakeIndexPage([ARGO_SYS_PARAM.folder_dest, 'IndexAU_Cycles.html'],          IndexPageInfo, 'Cycles'         );
%     HTMLmakeIndexPage([ARGO_SYS_PARAM.folder_dest, 'IndexAU_Voltage.html'],         IndexPageInfo, 'Voltage'        );
%     HTMLmakeIndexPage([ARGO_SYS_PARAM.folder_dest, 'IndexAU_Status.html'],          IndexPageInfo, 'Status'         );
%     HTMLmakeIndexPage([ARGO_SYS_PARAM.folder_dest, 'IndexAU_Ship.html'],            IndexPageInfo, 'Ship'           );
%     HTMLmakeIndexPage([ARGO_SYS_PARAM.folder_dest, 'IndexAU_System.html'],          IndexPageInfo, 'System'         );
%     HTMLmakeIndexPage([ARGO_SYS_PARAM.folder_dest, 'IndexAU_LastTx.html'],          IndexPageInfo, 'LastTx'         );
%     HTMLmakeIndexPage([ARGO_SYS_PARAM.folder_dest, 'IndexAU_TxDaysAgo.html'],       IndexPageInfo, 'TxDaysAgo'      );
%     HTMLmakeIndexPage([ARGO_SYS_PARAM.folder_dest, 'IndexAU_DaysOperational.html'], IndexPageInfo, 'DaysOperational');
% 
% %end
% 
% 
% 
% system(['chmod o+r ' folder_dest '*'])
% 
% 
% %====================================================================
% % RE-GENERATES AND UPDATES ALL WEB PAGES FOR ALL FLOATS:
% % INDEX PAGES ARE ALSO UPDATED:
% %====================================================================
% function AllWebPages()
% %begin
%     %DEFINE GLOBAL DIRECTORIES:
%     global FOLDER_HTML  FOLDER_DBASE % FOLDER_CONFIG
%     global ARGO_SYS_PARAM ARGO_ID_CROSSREF
% 
%     %load all filenames of deployed floats - changed to use
%     %ARGO_ID_CROSSREF - AT Sept 2010
%     warning off;
%     kk          = 0;
%     folder_dest = FOLDER_HTML; %'\\strait-hf\argoweb\tech\';
%     
% %     if (isempty(filenames))
% %         fprintf('Error: ArgoRT Directory Not Found. \n');
% %         return;
% %     end
%     
%     %create the html pages for each float:
%     for j=1:length(ARGO_ID_CROSSREF)
%         
%         wmo = ARGO_ID_CROSSREF(j,1);
% 
%         if (isempty(wmo)) continue; end;
%   
%         %make the web page, skip empty files:
%         A = webMakeSingleWebPage(wmo);
% %         fprintf('...completed \n');
...
%     end
% 
%     %save for later index page update ie: incremental buildup of individual web pages:
%     if (~isempty(IndexPageInfo)) save('IndexPageInfo.mat', 'IndexPageInfo'); end;
% 
%     %generate the index pages sorted in different columns:
% %     fprintf(' \n');
% %     fprintf('CONSTRUCTING INDEX PAGES: \n');
% %     fprintf('------------------------- \n');  
%     HTMLmakeIndexPage([folder_dest, 'index.html'],                   IndexPageInfo, 'HULLID'         );
%     HTMLmakeIndexPage([folder_dest, 'IndexAU_HullID.html'],          IndexPageInfo, 'HULLID'         );
%     HTMLmakeIndexPage([folder_dest, 'IndexAU_ArgosID.html'],         IndexPageInfo, 'ARGOSID'        );
%     HTMLmakeIndexPage([folder_dest, 'IndexAU_WmoID.html'],           IndexPageInfo, 'WMOID'          );
%     HTMLmakeIndexPage([folder_dest, 'IndexAU_LaunchDate.html'],      IndexPageInfo, 'LaunchDate'     );
%     HTMLmakeIndexPage([folder_dest, 'IndexAU_Cycles.html'],          IndexPageInfo, 'Cycles'         );
%     HTMLmakeIndexPage([folder_dest, 'IndexAU_Voltage.html'],         IndexPageInfo, 'Voltage'        );
%     HTMLmakeIndexPage([folder_dest, 'IndexAU_Status.html'],          IndexPageInfo, 'Status'         );
%     HTMLmakeIndexPage([folder_dest, 'IndexAU_Ship.html'],            IndexPageInfo, 'Ship'           );
%     HTMLmakeIndexPage([folder_dest, 'IndexAU_System.html'],          IndexPageInfo, 'System'         );
%     HTMLmakeIndexPage([folder_dest, 'IndexAU_LastTx.html'],          IndexPageInfo, 'LastTx'         );
%     HTMLmakeIndexPage([folder_dest, 'IndexAU_TxDaysAgo.html'],       IndexPageInfo, 'TxDaysAgo'      );
%     HTMLmakeIndexPage([folder_dest, 'IndexAU_DaysOperational.html'], IndexPageInfo, 'DaysOperational');
% %end
% 



%useless! never updated with new floats! over 100 behind!!! 
...
%=======================================================================================
% function HTMLmakeIndexPage(destfile, IndexPageInfo, SortBy)
%     global ARGO_SYS_PARAM
% %begin   
%     %CHECK INPUTS:
%     if (isempty(destfile))      return; end;
%     if (isempty(IndexPageInfo)) return; end;
%         
%     %CONSTRUCT THE HTML FILE: ADD DATE/TIME UNDER TITLE:
%     htmlsrc  = 'Template_Index.html';
%     htmlCode = LoadTextFile(htmlsrc); 
%     Data     = sortstruc(IndexPageInfo, 'HullID');
...
%                                    Data = sortstruc(IndexPageInfo, 'ArgosID');
%                            
%         case {'WMOID'};          %  fprintf(' [03] Constructing Index Table.....:  Sort By WMO ID \n');
%                                    Data = sortstruc(IndexPageInfo, 'WmoID');
%                            
...
%     htmlCode = ReplaceIndexTable(htmlCode, Data, SortBy);
