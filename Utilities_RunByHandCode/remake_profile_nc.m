...
%     if any(kk == THE_ARGO_FLOAT_DB(ii).wmo_id) %already done these in remake_Bprofile.nc
%         continue
%     end
...
%     if isfield(fpp,'Bphase_raw')%         dbdat.oxy
        disp(ii)
        for j=1:length(fpp)
            %             [ii j]
            ...
            try
                ...
                float = fpp;
                save(['/home/argo/ArgoRT/matfiles/float' num2str(dbdat.wmo_id) '.mat'],'float','-v6');
            end
            %copy to export
